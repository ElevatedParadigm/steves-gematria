#!/usr/bin/env python3
"""
🧙‍♂️ STEVE'S GEMATRIA KNOWLEDGE GRAPH - SQLite ALTERNATIVE
============================================================
Zero dependencies. Works immediately. Perfect for scalable research!
    ...
    def _initialize_schema(self, conn):
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS gematria_entities (
                ...
                primary_domain TEXT,
                secondary_domains TEXT,  -- JSON array stored as text
                embedding TEXT,          -- JSON vector for similarity search
                relevance_score REAL DEFAULT 0.0,
                source_url TEXT,
                first_seen TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                last_updated TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                -- No UNIQUE on symbol_value; use application-level check or add separate constraints as needed
            )
        """)
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS gematria_relationships (
                ...
                description TEXT,
                context_fields TEXT,      -- JSONB-like storage
                source_url TEXT,
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS analysis_metrics (
                ...
                domain_scores TEXT,         -- JSON object
                elemental_activations TEXT, -- JSON object  
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS overnight_research_logs (
                ...
                symbols_found TEXT,         -- JSON array
                domains_identified TEXT      -- JSON array
            )
        """)
        ...
        cursor.execute("""
            CREATE INDEX IF NOT EXISTS gematria_entities(primary_domain)
        """)
    ...
    def add_entity(self, **kwargs) -> int:
        ...
        try:
            with sqlite3.connect(str(self.db_path)) as conn:
                ...
                # Remove UNIQUE to allow multiple entities if needed
                cursor.execute("""
        ...
        try:
            with sqlite3.connect(str(self.db_path)) as conn:
                ...
                for row in rows:
                    ...
                    # Parse JSON fields back to Python types
        ...
        try:
            with sqlite3.connect(str(self.db_path)) as conn:
                ...
                if existing:
                    ...
                    # Update last updated timestamp via trigger or separate table would be ideal
                    cursor.execute("UPDATE gematria_relationships SET created_at = ? WHERE id = ?", 
...
def demo_sqlite_kg():
    ...
    kg.add_relationship(
        ...
        entity_b_id=None,  # Will be looked up if needed
