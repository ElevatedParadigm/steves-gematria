...
async def main():
    ...
    # Main autonomous loop with enhanced parallel support
        ...
        if remaining <= TIMEOUT_GRACE_PERIOD:
            ...
            try:
                if PARALLEL_SCRAPE_ENABLED:
                    ...
                    logger.info(f"🚀 Running in parallel scraping mode with {PARALLEL_WORKERS} workers")
                    
                    async with aiohttp.ClientSession(headers=HEADERS) as session:
                        chunk_size = max(len(urls) // PARALLEL_WORKERS, 1)
                        
                        tasks = []
                        for i, worker_id in enumerate(range(PARALLEL_WORKERS)):
                            ...
                            if start_idx < len(urls):
                                ...
                                # Build task for each parallel worker
                                task = session.post(
                                    ...
                                    json={
                                        ...
                                        "searchQueries": EXTERNAL_SEARCH_QUERIES  # External queries for all parallel workers
                                    }, 
                                    headers=HEADERS
                                )
                                tasks.append(task)
                                ...
                                for idx, resp in enumerate(responses):
                                    if not isinstance(resp, Exception):
                                        if isinstance(resp, dict) and "markdown" in resp:
                                            combined_markdown += resp["markdown"] + "\n\n---\n"
                                    
                            except Exception as task_error:
                                logger.warning(f"Parallel scraping task error: {str(task_error)[:80]}")
                                # Fall back to single worker if parallel fails
                                
                        # Analyze combined results
                        scrape_results = {"markdown": combined_markdown, "external_queries": EXTERNAL_SEARCH_QUERIES}
                    ...
                    scrape_results, status_msg = await scrape_urls_batch(
                ...
                symbol_count, domain_list, description = await analyze_patterns_in_results(scrape_results)
                ...
                results_logger.log_result(short_commit, 0, 0, len(EXTERNAL_SEARCH_QUERIES) if PARALLEL_SCRAPE_ENABLED else 0, 
                                         "crash", error_desc)
