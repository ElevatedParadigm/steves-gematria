...
try:
    ...
    StabilityCore = type('StabilityCore', (), {'handle_exception': lambda self, e: None})()
...
class AnomalyDetector:
    ...
    def detect_all_anomalies(self) -> List[AnomalyRecord]:
        ...
        for symbol_key, item in self.core_symbols.items():
            ...
            hist_items = self.core_symbols.get_all_items()
            historical_mean = sum(abs(i.get('temporal_variance', 0)) for i in hist_items) / len(hist_items) if hist_items else None
